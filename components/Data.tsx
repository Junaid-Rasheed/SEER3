...
export default function Data() {
  return (
    <div className="relative py-[100px] md:py-[200px]">
          ...
          {statistic.map(({ title, value }) => (
            <li key={title + value}>
              <div className="flex items-center">
                <DataCounting value={value} className="heading text-7xl md:text-[8vw] mb-2" />
                <span className="heading text-7xl md:text-[8vw] mb-2">+</span>
              </div>
              <p>{title}</p>
            </li>
          ))}
      ...
      <div className="absolute top-0 left-0 bg-data-mobile md:bg-data h-full w-full bg-cover" />
    </div>
  )
}
